...
import { submitItem } from '@/lib/archive/archive-service';
...
/* ------------------------------------------------------------------ */
/* POST /api/archive/admin/items/[id]/submit — enviar a revisión       */
/* ------------------------------------------------------------------ */
export const POST = guardedRoute<ArchivePublicationInput>(
  {
    route: 'api:archive:admin:items:submit',
    methods: ['POST'],
    rateLimit: 10,
  ...
  async ({ req, body }) => {
    ...
    const result = submitItem(actor, id, body.changeReason);
    if (!result.ok) {
