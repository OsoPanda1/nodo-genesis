        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md" onClick={() => setShowComposer(false)}>
          <div className="w-full max-w-md p-6 glass-panel rounded-2xl border border-cyan-500/40 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-800 focus:border-cyan-500 rounded-xl px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none"
            />
            <textarea
              value={newBody}
              ...
              rows={4}
              className="w-full bg-slate-900 border border-slate-800 focus:border-cyan-500 rounded-xl px-3 py-2 text-sm text-white placeholder-slate-500 focus:outline-none resize-none"
            />
          ...
          <div key={thread.id} className={`p-5 rounded-2xl glass-panel border transition-all ${thread.pinned ? 'border-amber-500/30' : 'border-white/10'}`}>
            ...
            <div className="flex items-center gap-4 pt-3 mt-3 border-t border-white/10 text-[11px] font-mono">
              <button
                ...
                className={`flex items-center gap-1.5 transition-all ${liked[thread.id] ? 'text-cyan-400' : 'text-slate-400 hover:text-white'}`}
              >
