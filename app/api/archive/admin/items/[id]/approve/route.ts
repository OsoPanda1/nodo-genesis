...
import { approveItem } from '@/lib/archive/archive-service';
...
/* ------------------------------------------------------------------ */
/* POST /api/archive/admin/items/[id]/approve — aprobar expediente     */
/* ------------------------------------------------------------------ */
export const POST = guardedRoute<ArchivePublicationInput>(
  {
    route: 'api:archive:admin:items:approve',
    methods: ['POST'],
    rateLimit: 10,
  ...
  async ({ req, body }) => {
    ...
    const result = approveItem(actor, id, body.changeReason);
    if (!result.ok) {
